...
from bluetti_mqtt.device_handler
...
default=5,
            type=int,
            help='The polling interval - set to
0 to poll as fast as possible')
        parser.add_argument(
...
'--ha-config',
            ...
            choices=['normal', 'none',
'advanced'],
            help='What fields to configure in Home Assistant -
defaults to most fields ("normal")')
        parser.add_argument(
            '-v',
...
args = parser.parse_args()
        setup_logging(logging.DEBUG if args.v else
logging.INFO)
...
mqtt_task.add_done_callback(self.background_tasks.discard)
        ...
        # Start bluetooth handler (manages
connections)
        addresses: List[str] = list(set(args.addresses))
        handler =
DeviceHandler(addresses, args.interval, bus)
        bluetooth_task =
...
def
handle_global_exception(loop, context):
    if 'exception' in context:
       
logging.error('Crashing with uncaught exception:', exc_info=context['exception'])
   
else:
        logging.error(f'Crashing with uncaught exception:
{context["message"]}')
    asyncio.create_task(shutdown(loop))
...
asyncio.AbstractEventLoop):
    ...
    tasks = [t for t in
asyncio.all_tasks() if t is not asyncio.current_task()]
    [task.cancel() for
task in tasks]
