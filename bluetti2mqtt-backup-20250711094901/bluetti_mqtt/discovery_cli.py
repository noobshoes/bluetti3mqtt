...
from bluetti_mqtt.bluetooth
import BluetoothClient, ModbusError, ParseError, BadConnectionError
from
bluetti_mqtt.core import ReadHoldingRegisters
from bluetti_mqtt.bluetooth.encryption import
is_device_using_encryption


def log_packet(output: TextIOWrapper, data: bytes,
...
def log_invalid(output: TextIOWrapper, err: Exception, command:
...
async def log_command(client:
BluetoothClient, command: ReadHoldingRegisters, log_file: TextIOWrapper):
   
response_future = await client.perform(command)
    try:
        response =
cast(bytes, await response_future)
        print(f'Device data readable at
{command.starting_address}')
        log_packet(log_file, response, command)
    except
(BadConnectionError, BleakError, ParseError) as err:
        print(f'Got an error
running command {command}: {err}')
        log_invalid(log_file, err, command)
   
except ModbusError as err:
        # This is expected if we attempt to access an
invalid address
        log_invalid(log_file, err, command)
...
something probably went wrong')
     ...
     enc = ' (bluetti, encrypted)' if
is_device_using_encryption(adv.manufacturer_data) else ''
...
description='Attempts to discover supported MODBUS ranges for undocumented Bluetti devices',
 ...
 epilog=textwrap.dedent("""\
    ...
    %(prog)s --scan
            ...
            Once you have found your device you can run
the discovery tool:
...
parser.add_argument(
        ...
        action='store_true',
        help='Turn on
encryption (refer to the scan output)'
    )
    parser.add_argument(
