...
from
bluetti_mqtt.bluetooth import BluetoothClient, ModbusError
...
async def read_address(client, address, size):
    command =
ReadHoldingRegisters(address, size)
...
async def main(args):
    client =
BluetoothClient(args.address, args.encrypted)
...
1:
            ...
            try:
                raw_data,
data = await read_address(client, address, size)
                size += 1
...
parser.add_argument(
        "address", metavar="ADDRESS", help="The device MAC to
connect to for discovery"
    )
